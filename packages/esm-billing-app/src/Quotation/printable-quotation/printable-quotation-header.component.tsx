...
import styles from './printable-quotation-header.scss';
import { useTranslation } from 'react-i18next';
import startCase from 'lodash-es/startCase';

interface PrintableQuotationHeaderProps {
  patientDetails: PatientDetails;
  facilityInfo: Record<string, any>;
}

const PrintableQuotationHeader: React.FC<PrintableQuotationHeaderProps> = ({ patientDetails, facilityInfo }) => {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.printableHeader}>
        <p className={styles.heading}>{t('quotation', 'Quotation')}</p>
        {logo?.src ? (
      ...
      <div className={styles.printableBody}>
        <div className={styles.billDetails}>
          <p className={styles.itemHeading}>{t('quotedTo', 'Quoted to')}</p>
...
export default PrintableQuotationHeader;
