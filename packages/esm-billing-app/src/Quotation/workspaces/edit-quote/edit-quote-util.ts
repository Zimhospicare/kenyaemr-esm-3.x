 ...
 */
export const createEditQuotePayload = (lineItem, data, bill, adjustmentReason: string) => {
  if (!lineItem?.uuid || !bill?.lineItems) {
    throw new Error('Invalid input: lineItem and Quote are required with valid properties');
  }
