...
import styles from './edit-quote.scss';

import classNames from 'classnames';
import {
  EditQuoteFormData,
  useDefaultEditQuoteFormValues,
  useEditQuoteFormSchema,
  useFormInitialValues,
} from './useEditQuoteFormSchema';
import { createEditQuotePayload } from './edit-quote-util';
import { processBillPayment } from '../../../billing.resource';
import { LineItem, MappedBill } from '../../../types';

type EditBillFormProps = DefaultWorkspaceProps & { lineItem: LineItem; bill: MappedBill };

export const EditQuoteForm: React.FC<EditBillFormProps> = ({
  lineItem,
  closeWorkspace,
  bill,
  promptBeforeClosing,
  ...
  const editQuoteFormSchema = useEditQuoteFormSchema();
  const defaultValues = useDefaultEditQuoteFormValues(lineItem, bill);
  ...
  const {
    ...
    defaultValues,
    resolver: zodResolver(editQuoteFormSchema),
    mode: 'all',
  });
  ...
  const onSubmit: SubmitHandler<EditQuoteFormData> = async (formData) => {
    const updateBill = createEditQuotePayload(lineItem, formData, bill, formData.adjustmentReason);
    try {
      const response = await processBillPayment(updateBill, bill.uuid);
      if (response.ok) {
        showSnackbar({
          title: t('billUpdate', 'Bill update'),
          subtitle: t('billUpdateSuccess', 'Quote update was successful'),
          kind: 'success',
          timeoutInMs: 5000,
        });
      }
      ...
      showSnackbar({
        title: t('billUpdate', 'Quote update'),
